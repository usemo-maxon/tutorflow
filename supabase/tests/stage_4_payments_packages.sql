...
select lives_ok($$select public.record_student_payment((select workspace_id from public.tutor_profiles where id = '51000000-0000-4000-8000-000000000001'), '52000000-0000-4000-8000-000000000001', 10000, 'PLN', now(), 'cash', 'Retry', '[]'::jsonb, 'stage4:payment:overpay')$$, 'payment retry is idempotent');
